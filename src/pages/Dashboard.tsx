...
import { mockAccounts, mockTransactions, formatCurrency, formatDate } from "@/lib/mock-data";

export default function Dashboard() {
  const totalBalance = mockAccounts.reduce((sum, a) => sum + a.balance, 0);
  const monthIncome = mockTransactions.filter((t) => t.type === "income").reduce((s, t) => s + t.amount, 0);
  const monthExpense = mockTransactions.filter((t) => t.type === "expense").reduce((s, t) => s + Math.abs(t.amount), 0);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{mockAccounts.length} contas conectadas</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {sortedCategories.map(([cat, total]) => {
              const catData = mockTransactions.find((t) => t.category === cat);
              return (
